  ...
  (window["webpackJsonp"] = window["webpackJsonp"] || []).push([["inscription-inscription-module"], {
    ...
    function srcAppInscriptionInscriptionPageTs(module, __webpack_exports__, __webpack_require__) {
      ...
      var InscriptionPage = /*#__PURE__*/function () {
        function InscriptionPage(alertService, authService, navCtrl, loadingController, appComponent) {
        ...
        _createClass(InscriptionPage, [{
          ...
          key: "inscription",
          value: function inscription(form) {
            var _this = this;

            this.present();
            this.authService.signUpUser(form.value.nom.toLocaleLowerCase(), form.value.prenom.toLocaleLowerCase(), form.value.email.toLocaleLowerCase(), form.value.telephone, form.value.password).then(function () {
              _this.dismiss();

              _this.appComponent.afficheMenu = false;

              _this.navCtrl.navigateForward('folder');
            }, function (error) {
              _this.dismiss();

              if (error.toString().search('another') > -1) _this.alertService.print('Cette adresse email est deja utilisée, veillez choisir une autre adresse email', 'danger');else if (error.toString().search('formatted') > -1) _this.alertService.print('Adresse email invalide. Veillez reesayer', 'danger');else if (error.toString().search('should') > -1) _this.alertService.print('Votre mot de passe doit avoir minimum 6 caracteres', 'danger');else _this.alertService.print('Une erreur est survenue, veillez reesayer plutard', 'danger');
            });
          }
          /* Animation */
